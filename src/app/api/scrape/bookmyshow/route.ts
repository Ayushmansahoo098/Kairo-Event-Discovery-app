...
export async function POST() {
  try {
    console.log("API Route triggered: Scrape BookMyShow events...");
    const result = await syncBMSEvents();

    if (result.success) {
      await triggerEmbeddingsSync();
      return NextResponse.json({
        success: true,
        message: `Successfully synchronized ${result.count} events from BookMyShow.`,
        count: result.count,
    ...
    console.error("BookMyShow Scraper API Route failed:", error);
