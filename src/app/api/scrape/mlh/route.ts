...
export async function POST() {
  try {
    console.log("API Route triggered: Scrape MLH hackathons...");
    const result = await syncMLHEvents();

    if (result.success) {
      await triggerEmbeddingsSync();
      return NextResponse.json({
        success: true,
        message: `Successfully synchronized ${result.count} events from MLH.`,
        count: result.count,
    ...
    console.error("MLH Scraper API Route failed:", error);
